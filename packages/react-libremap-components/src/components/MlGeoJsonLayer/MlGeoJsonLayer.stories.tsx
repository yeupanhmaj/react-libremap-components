...
const WithSymbolTemplate: any = (props: MlGeoJsonLayerProps) => {
	const mapHook = useMap({
		mapId: undefined,
	});
	...
	mapHook.map?.loadImage(wgMarker).then((res) => {
		if (!res?.data) {
			// biome-ignore lint/suspicious/noConsole: need error info
...
export const WithSymbol = WithSymbolTemplate.bind({});
